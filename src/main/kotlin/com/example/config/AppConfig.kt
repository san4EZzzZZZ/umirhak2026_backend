...
data class AppConfig(
    ...
    companion object {
        fun from(config: ApplicationConfig): AppConfig = AppConfig(
            ...
            emailServiceUrl = config.property("app.emailService.url").getString().trimEnd('/'),
            internalServiceToken = config.property("app.internal.serviceToken").getString(),
            frontendBaseUrl = config.property("app.frontendBaseUrl").getString().trimEnd('/'),
            passwordResetTtlMinutes = config.property("app.passwordReset.ttlMinutes").getString().toLong(),
